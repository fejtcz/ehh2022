...
bool readBatteryStatus()
{
    ...
    mqtt_battery.status=(uint8_t)(batteryStatusRxMessage[1]);
    SerialPrintf(DATA, "Battery - Voltage A:   %d\n", (uint16_t)(batteryStatusRxMessage[2] + batteryStatusRxMessage[3]*0x100));
    mqtt_battery.voltage_a=(uint16_t)(batteryStatusRxMessage[2] + batteryStatusRxMessage[3]*0x100);
    SerialPrintf(DATA, "Battery - Voltage B:   %d\n", (uint16_t)(batteryStatusRxMessage[4] + batteryStatusRxMessage[5]*0x100));
    mqtt_battery.voltage_b=(uint16_t)(batteryStatusRxMessage[4] + batteryStatusRxMessage[5]*0x100);
    if(batteryStatusRxMessage.length() == 12)                                                                           // G5 or G6 Transmitter.
    {
        ...
        mqtt_battery.runtime=(uint8_t)(batteryStatusRxMessage[8]);
        SerialPrintf(DATA, "Battery - Temperature: %d\n", (uint8_t)batteryStatusRxMessage[9]);
        mqtt_battery.temperature=(uint8_t)(batteryStatusRxMessage[9]);
    }
    else if(batteryStatusRxMessage.length() == 10)                                                                      // G6 Plus Transmitter.
    {
        SerialPrintf(DATA, "Battery - Runtime:     %d\n", (uint8_t)batteryStatusRxMessage[6]);
        mqtt_battery.runtime=(uint8_t)(batteryStatusRxMessage[6]);
        SerialPrintf(DATA, "Battery - Temperature: %d\n", (uint8_t)batteryStatusRxMessage[7]);
        mqtt_battery.temperature=(uint8_t)(batteryStatusRxMessage[7]);
    }
...
bool readGlucose()
{
    ...
    mqtt_glucose.status=status;
    mqtt_glucose.sequence=sequence;
    mqtt_glucose.timestamp=timestamp;
    mqtt_glucose.glucose=glucose;
    mqtt_glucose.state=state;
    mqtt_glucose.trend=trend;
    mqtt_glucose.sync=1;
